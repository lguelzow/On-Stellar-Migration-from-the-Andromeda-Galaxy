...
readout1 = np.loadtxt('present-time-half-mass.txt', dtype=np.str)
...
# Read out the data of all distances at t_0
# readout1 = np.loadtxt('all-min-distances-to-sun.txt', dtype=np.str)
readout2 = np.loadtxt('minimum-distances-half-mass.txt', dtype=np.str)
...
print(len(present_gal))
print(len(present_galcentre))
...
z = present_sun_distance
# plot the points as a scatter plot
star_pos = plt.scatter(present_gal.l.wrap_at('180d').radian,
                       present_gal.b.radian, s=15, vmin=0, vmax=40, c=z, cmap=cm, alpha=0.7)
plt.colorbar(star_pos, label='$r_{Sun}$ [kpc]', fraction=0.03, pad=0.03)
plt.title('Positions of simulation HVSs at $t_0$ \n Distance to Sun $r_{Sun}<40$ kpc \n Half-mass scenario, Gal. coord.',
          loc='right', fontsize=10)
# plt.legend(loc='upper right')
plt.savefig("Present-star-dist-colourmap-half-mass.pdf")
...
plt.colorbar(star_pos, label='$r_{Sun}$[kpc]', fraction=0.03, pad=0.04)
plt.title(
    'Positions of simulation HVSs at minimum distance to MW centre \n Distance to Sun $r_{Sun}<40$ kpc \n Half-mass scenario, Gal. coord.', loc='right', fontsize=10)
# plt.legend(loc='lower right')
plt.savefig("Min-star-dist-colourmap-half-mass.pdf")
# plt.show()
